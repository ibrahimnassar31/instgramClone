...
'use client'; // نحتاج إلى هذا لأن المكون يستخدم Hooks
...
import { postData,deleteData, fetchData } from '../hooks/useApi';
const Post = ({ post }) => {
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await fetchData(`https://instaclone-g9h5.onrender.com/api/v1/post/${post._id}/${action}`
      );
      if (res.data.success) {
  ...
  const deletePostHandler = async () => {
    try {
      const res = await deleteData(`https://instaclone-g9h5.onrender.com/api/v1/post/delete/${post._id}`);
      if (res.data.success) {
  ...
  const bookmarkHandler = async () => {
    try {
      const res = await fetchData(`https://instaclone-g9h5.onrender.com/api/v1/post/${post._id}/bookmark`);
      if (res.data.success) {
        toast.success(res.data.message);
      }
    } catch (error) {
